...
export const CippNinjaCveSyncScheduleDrawer = ({
  buttonText = "Schedule NinjaOne CVE Sync",
  requiredPermissions = [],
  ...
  const handleSubmit = () => {
    ...
    const shippedValues = {
      ...
      Name: `CIPP NinjaCveSync - ${tenantFilter}`,
      Command: { value: "Invoke-CIPPScheduledNinjaCveSync" },
      Parameters: {
        TenantFilter: tenantFilter,
        ScanGroupPrefix: values.scanGroupPrefix ?? "",
      },
  ...
  return (
    <>
      ...
      <CippOffCanvas
        title="Schedule NinjaOne CVE Sync"
      ...
      >
        <Box sx={{ my: 2 }}>
          <Alert severity="info" sx={{ mb: 3 }}>
            <AlertTitle>NinjaOne CVE Sync Schedule</AlertTitle>
            Schedule automatic upload of Defender TVM vulnerability data into NinjaOne for the
            selected tenant(s). CVEs with active CIPP exceptions are excluded from the upload.
          </Alert>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12 }}>
              <Typography variant="h6">NinjaOne Configuration</Typography>
            </Grid>

            <Grid size={{ xs: 12 }}>
              <CippFormComponent
                type="textField"
                label="Scan Group Name Prefix (optional)"
                name="scanGroupPrefix"
