...
const Offcanvas = (row, rowIndex, formatExtraData) => {
  ...
  const viewLink = `/identity/administration/Guests/view?userId=${row.id}&tenantDomain=${tenant.defaultDomainName}&userEmail=${row.userPrincipalName}`
  const editLink = `/identity/administration/Guests/edit?userId=${row.id}&tenantDomain=${tenant.defaultDomainName}`
  //console.log(row)
  return (
    <>
      <Link to={viewLink}>
...
const Guests = (row) => {
  const tenant = useSelector((state) => state.app.currentTenant)
  const titleButton = <TitleButton href="/identity/administration/guests/add" title="Add Guest" />
  return (
    <CippPageList
      ...
      datatable={{
        ...
        columns,
        path: '/api/ListGuests',
        reportName: `${tenant?.defaultDomainName}-Guests`,
