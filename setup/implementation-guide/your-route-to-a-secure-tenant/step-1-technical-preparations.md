...
You can edit a baseline by clicking the actions button and choosing edit, there are cases in which you might want to edit the baseline immediately, for example you've already agreed with your customers to enable the Unified Audit log and don't need permission for these changes.&#x20;

If that's the case, edit the template and set each item you want automatically resolved to "Auto remediate when drift is detected" - This will force the tenant to be in sync with your template immediately. This also means that if a client changes a setting, our standard will change this setting back immediately.

Let's move on to the next step; reporting on the current state of the tenant and what changes we'll want to make.
